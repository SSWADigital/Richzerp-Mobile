...
get() async {
  ...
  Future Jenis() async {
    ...
    final Response = await http.get(
      Uri.parse("${url}/${urlSubAPI}/Izin/getJenisIzin/$idDep"),
    );
  ...
  Future _insertDatacuti() async {
    ...
    var id_dep = prefs.getString('departemen_id');
    final urls = Uri.parse(
        '${url}/${urlSubAPI}/izin/addData'); // Ganti dengan URL endpoint API Anda

    try {
      var request = http.MultipartRequest('POST', urls);
      request.fields.addAll({
        'izin_tgl_mulai': tgl_awal_ymd ?? "",
        'izin_tgl_selesai': tgl_awal_ymd ?? "",
        'id_jenis_izin': bagian_jenis ?? "",
        'id_user': idUser.text,
        'izin_keterangan': keterangan.text ?? "-",
        'id_dep': id_dep ?? "",
        'izin_jam_mulai': selectedStartTime ?? "",
        'izin_jam_selesai': selectedEndTime ?? "",
      });
                        ...
                        Ink(
                          ...
                          child: Stack(
                            children: [
                              ...
                              // Tombol hapus ikon setengah di dalam dan setengah di luar
                              Positioned(
                                top: -1,
                                left: 130,
                                child: Container(
