...
class approvalIzinPage extends StatefulWidget {
  @override
  _approvalIzinPageState createState() => _approvalIzinPageState();
}
...
// Fungsi untuk memfilter data izin berdasarkan status
void filterIzin(String selectedStatus) async{
    await ambildata();
  ...
  if (selectedStatus == 'Semua') {
    ...
    filteredJadwal = jadwal?.where((item) {
      return item['izin_status'] == selectedStatus; 
    }).toList();
  }
...
void _filterApprovalIzin() {
  if (selectedDateRange != null) {
    List? filteredJadwal = jadwal?.where((item) {
      DateTime izinMulai = DateTime.parse(item['izin_tgl_mulai']);
      DateTime izinSelesai = DateTime.parse(item['izin_tgl_selesai']);
      return izinMulai.isAfter(selectedDateRange!.start.subtract(Duration(days: 1))) &&
          izinSelesai.isBefore(selectedDateRange!.end.add(Duration(days: 1)));
    }).toList();
...
Widget statusIzin(String status) {
return Container(
  decoration: BoxDecoration(
    color: status == "y" ? Colors.green : status == "n" ? Colors.orange : Colors.red,
    borderRadius: BorderRadius.circular(5),
  ),
  padding: EdgeInsets.symmetric(horizontal: 10, vertical: 5),
  child: Text(
    status == "y" ? "Disetujui" : status == "n" ? "Menunggu" : "Ditolak",
    style: TextStyle(
      color: Colors.white,
  ...
  Future ambildata() async {
    ...
    final Response = await http.get(
      Uri.parse("${url}/${urlSubAPI}/Izin/getIzinHistoryByIdDep/$idDep"),
    );
                ...
                child: Stack(
                  children: [
                    Padding(
                      ...
                      child: ListTile(
                        title: Text(
                          "${jw['user_nama_lengkap']} | ${formatDate(jw['izin_created'])}",
                          style: TextStyle(
                            fontSize: 15,
                            fontFamily: "Inter",
                            fontWeight: FontWeight.w700,
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            Text(
                              "${formatDate(jw['izin_tgl_mulai'])} - ${formatDate(jw['izin_tgl_selesai'])}",
                              style: TextStyle(
                                fontSize: 14,
                        ...
                        onTap: () {
                          // Pindah ke halaman detail izin
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => DetailIzinPage(jw: jw),
                            ),
                          );
                        },
                      ),
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: _getStatusColor(jw['izin_status']),
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child: Row(
                          children: [
...
class DetailIzinPage extends StatelessWidget {
  final Map<String, dynamic> jw; // Menerima data izin

  // Konstruktor
  DetailIzinPage({required this.jw});
...
return Container(
  ...
  Future<void> _downloadFile(String fileUrl, BuildContext context) async {
    ...
    if (permissionStatus.isGranted) {
      try {
        ...
        String savePath = '${dir.path}/izin_image.jpg';

        Dio dio = Dio();
        await dio.download(fileUrl, savePath);

        bool? isSaved = await GallerySaver.saveImage(savePath);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to download and save image')),
        );
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
   ...
   _approveIzin(BuildContext context, String keterangan) async {
    // print("APROVE");
    final prefs = await SharedPreferences.getInstance();
    ...
    http.post(
      Uri.parse("${url}/${urlSubAPI}/Izin/approveIzin"),
  ...
  Navigator.push(
  context,
  MaterialPageRoute(builder: (context) => approvalIzinPage()),
);
        ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('Izin berhasil disetujui')),
  );
          // Navigator.pop(context);
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal menyetujui izin')),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
   ...
   _rejectIzin(BuildContext context, String keterangan) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    http.post(
  Uri.parse("${url}/${urlSubAPI}/Izin/rejectIzin"),  // Change this line
  headers: <String, String>{
    'Content-Type': 'application/json; charset=UTF-8',
  },
  body: jsonData,
...
ScaffoldMessenger.of(context).showSnackBar(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal menolak izin')),
      );
    }
  } else {
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: getStatusColor(jw['izin_status']),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Text(
                  statusText(jw['izin_status']),
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
              ...
              detailRow("Tanggal", "${formatDate(jw['izin_tgl_mulai'])} - ${formatDate(jw['izin_tgl_selesai'])}"),
              detailRow("Jam", "${jw['izin_jam_mulai']} - ${jw['izin_jam_selesai']}"),
              detailRow("Keterangan", jw['izin_keterangan'] ?? "-"),
              detailRow("Jenis Izin", jw['jenis_izin_nama'] ?? "-"),
              detailRow("Keterangan HRD", jw['izin_keterangan_hrd'] ?? "-"),
              SizedBox(height: 16),
              jw['izin_file'] == null
              ...
              SizedBox(height: 10),
              if (jw['izin_file'] != null)
                Center(
                  child: ElevatedButton(
                    onPressed: () {
                      _downloadFile(jw['izin_file'], context);
                    },
      ...
      bottomNavigationBar: buildBottomNavigationBar(context, jw['izin_status']),
    );
  }
        ...
        child: Row(
          children: [
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  showConfirmationDialog(context, "Apakah Anda yakin ingin menyetujui izin ini?", "y");
                },
                style: ElevatedButton.styleFrom(
            ...
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  showConfirmationDialog(context, "Apakah Anda yakin ingin menolak izin ini?", "x");
                  // _rejectIzin(context, keterangan.text);
                },
                style: ElevatedButton.styleFrom(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                Navigator.pop(context);
                if (type == "y") {
                  await _approveIzin(context, keterangan.text);
                } else {
                  await _rejectIzin(context, keterangan.text);
                }
                // await _approveIzin(context, keterangan.text);
              },
              child: Text("Ya"),
