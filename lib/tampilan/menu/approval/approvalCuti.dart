...
class approvalcutiPage extends StatefulWidget {
  @override
  _approvalcutiPageState createState() => _approvalcutiPageState();
}

class _approvalcutiPageState extends State<approvalcutiPage> {
...
// Fungsi untuk memfilter data cuti berdasarkan status
void filtercuti(String selectedStatus) async{
    await ambildata();
  ...
  if (selectedStatus == 'Semua') {
    ...
    filteredJadwal = jadwal?.where((item) {
      return item['cuti_status'] == selectedStatus; 
    }).toList();
  }
...
void _filterApprovalcuti() {
  if (selectedDateRange != null) {
    List? filteredJadwal = jadwal?.where((item) {
      DateTime izinMulai = DateTime.parse(item['cuti_tgl_mulai']);
      DateTime izinSelesai = DateTime.parse(item['cuti_tgl_selesai']);
...
Widget statuscuti(String status) {
return Container(
  ...
  Future ambildata() async {
    ...
    final Response = await http.get(
      Uri.parse("${url}/${urlSubAPI}/cuti/getcutiHistoryByIdDep/$idDep"),
    );
                ...
                child: Stack(
                  children: [
                    Padding(
                      ...
                      child: ListTile(
                        title: Text(
                          "${jw['user_nama_lengkap']} | ${formatDate(jw['cuti_created'])}",
                          style: TextStyle(
                            fontSize: 15,
                            fontFamily: "Inter",
                            fontWeight: FontWeight.w700,
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            Text(
                              "${formatDate(jw['cuti_tgl_mulai'])} - ${formatDate(jw['cuti_tgl_selesai'])}",
                              style: TextStyle(
                                fontSize: 14,
                        ...
                        onTap: () {
                          // Pindah ke halaman detail cuti
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => DetailcutiPage(jw: jw),
                            ),
                          );
                        },
                      ),
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: _getStatusColor(jw['cuti_status']),
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child: Row(
                          children: [
...
class DetailcutiPage extends StatelessWidget {
  final Map<String, dynamic> jw; // Menerima data cuti

  // Konstruktor
  DetailcutiPage({required this.jw});
...
return Container(
  ...
  String formatDate(String dateStr) {
   ...
   _approvecuti(BuildContext context, String keterangan) async {
    // print("APROVE");
    final prefs = await SharedPreferences.getInstance();
    ...
    http.post(
      Uri.parse("${url}/${urlSubAPI}/cuti/approvecuti"),
  ...
  Navigator.push(
  context,
  MaterialPageRoute(builder: (context) => approvalcutiPage()),
);
        ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('Cuti berhasil disetujui')),
  );
          // Navigator.pop(context);
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal menyetujui cuti')),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
   ...
   _rejectcuti(BuildContext context, String keterangan) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    http.post(
  Uri.parse("${url}/${urlSubAPI}/cuti/rejectcuti"),  // Change this line
  headers: <String, String>{
    'Content-Type': 'application/json; charset=UTF-8',
  },
  body: jsonData,
  ...
  MaterialPageRoute(builder: (context) => approvalcutiPage()),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('cuti berhasil ditolak')),
  );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal menolak cuti')),
      );
    }
  } else {
    ScaffoldMessenger.of(context).showSnackBar(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: getStatusColor(jw['cuti_status']),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Text(
                  statusText(jw['cuti_status']),
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
              ...
              detailRow("Tanggal", "${formatDate(jw['cuti_tgl_mulai'])} - ${formatDate(jw['cuti_tgl_selesai'])}"),
              // detailRow("Jam", "${jw['cuti_jam_mulai']} - ${jw['cuti_jam_selesai']}"),
              detailRow("Keterangan", jw['cuti_keterangan'] ?? "-"),
              detailRow("Jenis cuti", jw['jenis_cuti_nama'] ?? "-"),
              detailRow("Keterangan HRD", jw['cuti_keterangan_hrd'] !=null  ? "${jw['cuti_keterangan_hrd']} (${jw['user_hrd']})" : "-"),
              ],
          ),
        ),
      ),
      bottomNavigationBar: buildBottomNavigationBar(context, jw['cuti_status']),
    );
  }
          ...
          children: [
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  showConfirmationDialog(context, "Apakah Anda yakin ingin menyetujui cuti ini?", "p");
                },
                style: ElevatedButton.styleFrom(
            ...
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  showConfirmationDialog(context, "Apakah Anda yakin ingin menolak cuti ini?", "x");
                  // _rejectcuti(context, keterangan.text);
                },
                style: ElevatedButton.styleFrom(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                Navigator.pop(context);
                if (type == "p") {
                  await _approvecuti(context, keterangan.text);
                } else {
                  await _rejectcuti(context, keterangan.text);
                }
                // await _approvecuti(context, keterangan.text);
              },
              child: Text("Ya"),
